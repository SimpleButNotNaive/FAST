...
def fair_dynamic_train(meta, total_xs, total_ys, 
                       genders, gamma, num_epoch, batch_sz, tau, device):
    ...
    # ---------------------------------
        ...
        p0 -= sign * min(gamma * (F_mean_loss - M_mean_loss).abs().item(), tau)
...
def test(meta, total_xs, total_ys, genders, device):
    ...
    return (torch.stack(mae_losses).mean().item(), 
            torch.stack(mse_losses).mean().item(), abs(M_mean_mae - F_mean_mae),
            abs(M_mean_mse - F_mean_mse))